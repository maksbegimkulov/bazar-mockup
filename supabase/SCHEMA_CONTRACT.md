...
Обе эти функции выданы только `authenticated` и `service_role`. У гостя
сохранённых поисков нет, а сводка без сессии означает «разобрать всех» — это
работа расписания, не браузера; клиент их без входа и не зовёт. Внутри
`rpc_saved_search_new_counts` пустой `auth.uid()` всё равно обработан (пустой
объект, а не ошибка) — это про служебную роль, а не про доступ для `anon`.
...
**`revoke ... from public` ничего не закрывает.** На Supabase шаблонные права
проекта (`alter default privileges`) выдают `execute` на новые функции напрямую
ролям `anon` и `authenticated`, а эти выдачи снимаются только поимённо. Обожглись
на боевой 15.08.2026: после первого наката схемы гость мог позвать и сырую
выборку цен, и `rpc_flush_saved_search_digest` — а сводка без сессии по замыслу
означает «разобрать всех», то есть рассылку чужих уведомлений. Поэтому в каждом
блоке прав сначала `revoke all ... from public`, **затем отдельно от `anon` и
`authenticated`** (обе строки — под проверкой `pg_roles`, чтобы миграция шла и на
голом Postgres), и только потом `grant` тем, кому функция действительно нужна.
Триггеры это не ломает: права на триггерную функцию проверяются при создании
триггера, а не на каждой строке. Итог проверяет группа тестов «Права на функции»
— она сама воспроизводит боевую щедрость, потому что локальный стек создаёт
функции от `postgres` и такой проблемы не показывает.
