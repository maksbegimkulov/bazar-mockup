-- BAZAR · единый накат схемы (21 миграций, автосборка). Идемпотентно, безопасно поверх боевой базы.
...
declare
  ...
  v_res jsonb;
begin
  -- У гостя сохранённых поисков нет; пустой объект, а не ошибка — список
  -- рисуется и до входа (из localStorage).
  if v_uid is null then
    return '{}'::jsonb;
  end if;
