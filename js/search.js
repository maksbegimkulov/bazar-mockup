...
function parseSearchQuery(raw) {
  // диапазон «30000-60000», «50-80 тыс»: дефис гибнет в normText,
  // поэтому заранее переписываем в «от X .. до Y ..»
  const preranged = String(raw).replace(
    /(\d[\d\s]*?)\s*[-–—]\s*(\d[\d\s]*)\s*(к|k|тыс[а-яa-z]*|млн[а-яa-z]*)?/i,
    'от $1 $3 до $2 $3'
  );
  ...
  if (m && !filters.priceMax) {
    ...
    if (v && v > 50) { filters.priceMax = String(v); s = s.replace(m[0], ' '); }
  }
  // «от 100 000», «дороже 1 млн»
  m = s.match(new RegExp('(?:^| )(?:от|дороже|не дешевле|минимум)\\s+' + AMOUNT_RE));
  if (m && !filters.priceMin) {
    const v = parseAmount(m[1], m[2]);
    if (v && v > 10) { filters.priceMin = String(v); s = s.replace(m[0], ' '); }
  }
  // «50-80 тыс»: юнит относится к обеим границам
  if (filters.priceMin && filters.priceMax && +filters.priceMin * 1000 <= +filters.priceMax) {
    filters.priceMin = String(+filters.priceMin * 1000);
  }
  ...
  if (/ (?:новый|новая|новое|новые|new)(?= )/.test(s)) {
    filters.condition = 'new';
    s = s.replace(/ (?:новый|новая|новое|новые|new)(?= )/g, ' ');
  }
  ...
  const intentOf = piece => {
    ...
    if (piece.length > 3) cands.push(piece.slice(0, -1));
    if (piece.length > 4) cands.push(piece.slice(0, -2));
...
function aliasFor(w) {
  ...
  if (w.length >= 4 && /^[а-я]+$/.test(w)) {
    for (const k in QUERY_ALIASES) {
      if (k.length >= 4 && fuzzyEq(w, k)) return QUERY_ALIASES[k];
    }
  }
  return w;
}
