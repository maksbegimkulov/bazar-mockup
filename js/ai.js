...
function isAdvisoryQuery(raw, f) {
  ...
  const s = normText(raw);
  if (f && f._gift) return true;               // подарочный интент распознан парсером
  if (detectPurpose(raw, f.cat)) return true;
  return /подар|сюрприз|презент|(?:^| )для |похож|лучш|надёжн|надежн|качествен|посоветуй|что.?нибудь|какой |какую |выбрать|стоит ли|недорог|подешевл|хорош(?:ая|ий|ей|ую) (?:камер|батар|состоян)/.test(s);
}
