...
const _GENERIC_SUFFIX = new Set([
  ...
  // слова-состояния / общие: не должны быть моделью каталога («новый»→Geely Atlas)
  'новый', 'новая', 'новое', 'новые', 'новый 2023', 'старый', 'бу', 'new',
  // «класс» сам по себе — не модель (Mercedes C-Class ловил «1 класс» школьной формы)
  'класс', 'class', 'классе', 'кубов', 'куб',
  // «мото» одиночное = мотоцикл (CAT_INTENT), НЕ бренд Motorola (тот в связке «мото g54»)
  'мото', 'moto',
]);
...
const SEMANTIC_HINTS = [
  ...
  // топливо: значения совпадают с O_FUEL/генератором
  [/ дизел[а-я]*(?= )/, { cat: 'transport', sub: 'Легковые авто', attrs: { fuel: 'Дизель' } }],
  [/ бензин[а-я]*(?= )/, { cat: 'transport', sub: 'Легковые авто', attrs: { fuel: 'Бензин' } }],
  // растаможка: «не растаможен» ПЕРЕД «растаможен» (совпадение вырезается,
  // поэтому второй не поймает то же слово)
  [/ (?:не растаможен|без растаможк|без учёт|без учет)[а-я]*(?= )/, { cat: 'transport', sub: 'Легковые авто', attrs: { customs: 'Не растаможен' } }],
  [/ (?:растаможен|растаможк|на учёт|на учет)[а-я]*(?= )/, { cat: 'transport', sub: 'Легковые авто', attrs: { customs: 'Растаможен' } }],
